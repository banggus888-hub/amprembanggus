...
<head>
    ...
    <style>
        ...
        .phone-wrapper {
            ...
            padding: 1rem; position: relative; box-sizing: border-box; overflow-x: hidden;
        }
        .glass-panel {
            background: rgba(13, 15, 23, 0.85); backdrop-filter: blur(20px);
        ...
        .input-glow:focus {
            border-color: rgba(16, 185, 129, 0.6); box-shadow: 0 0 20px rgba(16, 185, 129, 0.15); outline: none;
        }
        .cyber-btn {
            background: linear-gradient(135deg, #10b981 0%, #059669 100%);
            box-shadow: 0 10px 25px -5px rgba(16, 185, 129, 0.4); transition: all 0.3s ease;
        }
        .cyber-btn:hover { transform: translateY(-2px); box-shadow: 0 15px 30px -5px rgba(16, 185, 129, 0.6); }
        @keyframes pulseGlow { 0%, 100% { opacity: 0.4; } 50% { opacity: 0.8; } }
        .glow-bg { animation: pulseGlow 6s infinite ease-in-out; }
        #nav-drawer { transition: transform 0.3s ease-in-out; transform: translateX(100%); }
...
<body class="min-h-screen text-slate-100 selection:bg-emerald-500 selection:text-black">
    ...
    <div class="phone-wrapper z-10">
        ...
        <div id="nav-drawer" class="fixed inset-y-0 right-0 z-50 w-full max-w-[300px] bg-[#090b10]/95 backdrop-blur-xl border-l border-white/10 p-5 flex flex-col justify-between shadow-2xl">
            <div class="space-y-6">
                ...
                <nav class="space-y-2 text-xs font-semibold">
                    <button onclick="switchView('generator')" class="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-emerald-500/10 hover:text-emerald-400 text-slate-300 transition text-left"><span>⚡</span> Generator Utama</button>
                    <button onclick="switchView('profile')" class="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-emerald-500/10 hover:text-emerald-400 text-slate-300 transition text-left"><span>👤</span> Halaman Akun & Profil</button>
                    <button onclick="switchView('guide')" class="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-emerald-500/10 hover:text-emerald-400 text-slate-300 transition text-left"><span>📖</span> Panduan Penggunaan</button>
                    <button onclick="switchView('announcement')" class="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-emerald-500/10 hover:text-emerald-400 text-slate-300 transition text-left"><span>📢</span> Informasi & Pengumuman</button>
                </nav>
            </div>
            <div class="pt-4 border-t border-white/10 space-y-2">
                <div id="drawer-user-info" class="text-[10px] text-slate-400 truncate">Status: <span id="drawer-status-role" class="text-emerald-400 font-bold">Belum Login</span></div>
                <button onclick="handleLogout()" id="drawer-logout-btn" class="w-full py-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs font-bold hover:bg-rose-500/20 transition hidden">Keluar / Logout</button>
            </div>
        </div>
        ...
        <div class="w-full my-auto py-2 space-y-5">
            <div id="offline-banner" class="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-semibold text-center hidden">🔴 Server sedang dalam mode OFFLINE. Fitur premium dinonaktifkan untuk User & VIP.</div>
            ...
            <div id="section-profile" class="glass-panel p-6 rounded-3xl space-y-5 hidden">
                ...
                <div class="space-y-2.5 pt-2 border-t border-white/10">
                    <label class="text-[11px] font-bold tracking-wider text-slate-400 block flex items-center gap-1.5"><span>🎁</span> Klaim Kode Redeem Kuota</label>
                    <div class="flex gap-2">
                ...
                <div id="admin-control-panel" class="p-5 rounded-2xl bg-amber-500/10 border border-amber-500/30 space-y-4 hidden pt-4">
                    <p class="text-xs font-extrabold text-amber-400 flex items-center gap-2"><span>👑</span> Admin Master Control Panel</p>
  ...
  if (parsedUrl.pathname === '/' && req.method === 'GET') {
    ...
    req.on('end', async () => {
      try {
        ...
        if (mode === 'register') {
          ...
          const newUserData = { 
            password, email, isAdmin: false, activatedEmails: [], bonusQuota: 0, 
            lastResetTime: Date.now(), vipUntil: 0, deviceToken: newDeviceToken
          };
          await saveUserToDb(cleanUser, newUserData);
          const fakeAuthToken = 'token_' + Math.random().toString(36).substring(2) + Date.now();
          res.writeHead(200);
          res.end(JSON.stringify({ 
            success: true, message: 'Registrasi berhasil!', username: cleanUser, isAdmin: false, 
            usedQuota: 0, bonusQuota: 0, isVip: false, serverStatus, deviceToken: newDeviceToken, token: fakeAuthToken
          }));
        } else {
          if (existingUser && existingUser.password === password) {
            ...
            res.writeHead(200);
            res.end(JSON.stringify({ 
              success: true, message: 'Login berhasil!', username: cleanUser, isAdmin: existingUser.isAdmin, 
              usedQuota: usedCount, bonusQuota: existingUser.bonusQuota || 0, isVip: isVipActive, 
              vipUntil: existingUser.vipUntil || 0, serverStatus, token: fakeAuthToken
            }));
          } else {
            res.writeHead(401);
    ...
    req.on('end', async () => {
      try {
        ...
        if (serverStatus !== 'online' && (!userObj || !userObj.isAdmin)) {
          res.writeHead(403);
          res.end(JSON.stringify({ error: 'Server sedang offline.' }));
          return;
        }
        if (!email || !verifyUrl) {
          res.writeHead(400);
...
// Export handler agar kompatibel dengan Vercel Serverless Function
